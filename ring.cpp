...
int main(int argc,char **argv) {
	cout << "Program |ring| is running." << endl;
	...
	/* Getting arguments */
	if (argc < 10 || argc > 14) {
		cerr << "ERROR : Wrong arguments for ring.cpp!" << endl;
    ...
    float r1 = utilityArgs[2] * utilityArgs[2];
    float r2 = utilityArgs[3] * utilityArgs[3];
    if ( ifile.is_open() ) {
    	if (offsetExists) {
		...
		while (!ifile.eof()) {
			...
			if ( (dx * dx + dy * dy <= r1) && (dx * dx + dy * dy >= r2) )
